...
def test_path_dependent_shap_iv_using_shap_package_is_same_as_using_woodelf_explainer(trainset, testset, xgb_model):
    ...
    for i in range(len(trainset.columns)):
        # np.testing.assert_allclose(woodelf_values_not_including_self[:,i,i], shap_package_values_not_including_self[:,i,i], atol=TOLERANCE, strict=True)
        woodelf_values_not_including_self[:,i,i] = 0
        shap_package_values_not_including_self[:,i,i] = 0

    np.testing.assert_allclose(woodelf_values_not_including_self, shap_package_values_not_including_self, atol=TOLERANCE, strict=True)
    # E       Mismatched elements: 2502 / 1576090 (0.159%)
    np.testing.assert_allclose(woodelf_values, shap_package_values, atol=TOLERANCE, strict=True)
